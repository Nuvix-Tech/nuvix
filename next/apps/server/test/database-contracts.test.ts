...
describe("database schema request contracts", () => {
  test.each(["a", "appdata", `a${"0".repeat(254)}`])(
    "accepts schema name %s",
    (name) => {
  ...
  test.each(["document", "managed", "unmanaged"])(
    "accepts schema type %s",
    (type) => {
  ...
  test.each([
    ...
    "rejects invalid description payload %p with 422-compatible validation",
    (body) => {
  ...
  test.each([
    [
      CreateSchemaBody,
      { name: "appdata", type: "managed", collection: "posts" },
    ],
