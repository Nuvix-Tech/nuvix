...
async function requireSuccess(
  ...
  if (result?.exitCode !== 0) {
    throw new PostgresResourceError(
      `PostgreSQL test resource ${operation} failed`,
    );
  }
  return result;
}
...
function port(output: string): number {
  ...
  if (!Number.isInteger(selected) || selected < 1 || selected > 65_535) {
    throw new PostgresResourceError(
      "PostgreSQL test resource port discovery failed",
    );
  }
  return selected;
}
...
function owner(port: number, password: string): PostgresOwnerConnection {
  return Object.freeze({
    connectionString: () => {
      const connection = new URL(
        `postgresql://${POSTGRES_HOST}:${port}/${POSTGRES_DATABASE}`,
      );
...
export async function startPostgresResource(
  ...
  try {
    ...
    return Object.freeze({ owner: owner(ownerPort, password), close });
  } catch (error) {
