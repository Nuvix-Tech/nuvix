...
import type { DocumentSchemaBootstrap, DocumentSchemaInput } from '../src/database/document-schema'
...
function harness(options: HarnessOptions = {}) {
  ...
  const catalog: SchemaCatalog = {
    list: async (type) => {
      ...
      return [...schemas.values()].filter((schema) => type === undefined || schema.type === type)
    },
    get: async (name) => {
...
describe('schema service', () => {
  test('lists schemas with the exact v2 envelope and forwards the type filter', async () => {
    const state = harness({
      schemas: [MANAGED_SCHEMA, { name: 'documents', description: null, type: 'document' }],
    })
  ...
  test('creates a document schema with a null description and bootstraps metadata', async () => {
    ...
    expect(state.calls.bootstraps).toEqual([{ name: 'documents', type: 'document' }])
  })
  ...
  test.each(['managed', 'unmanaged'] as const)(
    ...
    async (type) => {
      ...
      expect(await state.service.create({ name: type, description: 'Data', type })).toEqual({
        name: type,
        description: 'Data',
        type,
      })
  ...
  test('updates a schema description to null', async () => {
    ...
    expect(state.calls.updates).toEqual([{ name: 'appdata', description: null }])
  })
  ...
  test.each([
    ['get', (state: ReturnType<typeof harness>) => state.service.get('missing')],
    ['update', (state: ReturnType<typeof harness>) => state.service.update('missing', 'Missing')],
    ['delete', (state: ReturnType<typeof harness>) => state.service.remove('missing')],
  ] as const)('returns schema_not_found when %s targets a missing name', async (_name, run) => {
    const state = harness()
  ...
  test('cleans up a document schema when metadata bootstrap fails', async () => {
    const bootstrapFailure = new DatabaseException('secret @nuvix/db bootstrap failure')
  ...
  test('keeps the bootstrap failure primary when cleanup also fails', async () => {
    const bootstrapFailure = new DuplicateException('secret primary bootstrap failure')
    ...
    expect(state.calls.removals).toEqual(['documents'])
    expect(problem(failure).fields.detail).toBe('Unable to initialize document schema')
  ...
  test('fails closed when the PostgreSQL catalog exposes an unexpected package failure', async () => {
    ...
    const exposedFailure = await service.list().catch((error: unknown) => error)
  ...
  test('exposes only the frozen schema operations', () => {
    ...
    expect(Object.keys(state.service)).toEqual(['list', 'get', 'create', 'update', 'remove'])
